...
let current; //current cell visited
let start;
let goal;
...
class Maze {
  ...
  draw() {
    ...
    maze.style.background = "#444444";    //"black";
...
class Cell {
  ...
  removeWalls(cell1, cell2) {
    ...
    if (x === 1) { //the neighbour cell is in the left
  ...
  show(size, rows, columns) {
    ...
    if (this.start) {
        ctx.fillStyle = "rgb(98, 215, 112)";
        ctx.fillRect(1, 1, size / columns - 2, size / rows - 2);
    }
    if (this.goal) {
